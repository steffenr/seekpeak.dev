...
| `config.json` | Single source of truth: `peakWindows` + `models` (deepseek-v4-flash, deepseek-v4-pro) with `cacheHit`/`cacheMiss`/`output` offPeak/peak prices (optional per-model `note` string renders under the id — `deepseek-v4-pro` carries "routes to V4.1 Flash on Sep 14, 2026"), + `site` block (`url`, `name`) for the SEO/OG head. |
