...
- `config.json` models take an optional `note` string; `renderPriceTable` renders it under the model `id`. Used for `deepseek-v4-pro` ("routes to V4.1 Flash on Sep 14, 2026") — from 04:00 UTC on that date DeepSeek routes Pro requests to V4.1 Flash and bills at Flash rates; until then Pro keeps its own (unchanged) prices. The homepage Pricing section carries a matching callout.
