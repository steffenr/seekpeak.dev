...
- Cross-page offer wiring: the homepage and the `omp` `#creditCta` link to `/free-credits/` (the offers hub), and **no page footer links `/agentrouter/`** — `verify.cjs` enforces both. Every page footer also carries an icon-only `<a href="https://github.com/steffenr/seekpeak.dev">` (inline GitHub mark SVG, `currentColor`) — `verify.cjs` checks all four. `free-credits.gist.md` is a standalone Markdown copy of that page for posting as a GitHub Gist; it is not built.
