...
- Cross-page offer wiring: the homepage and the `omp` `#creditCta` link to `/free-credits/` (the offers hub), and **no page footer links `/agentrouter/`** — `verify.cjs` enforces both. Every page footer also carries an icon-only `<a href="https://github.com/steffenr/seekpeak.dev">` (inline GitHub mark SVG, `currentColor`) — `verify.cjs` checks all four.
- **`free-credits` offers are data, not markup.** `credits.json` (repo root) is the source of truth — `scripts/credits.mjs` renders it into the `<!--__OFFER_CARDS__-->` block, the FAQ `__FAQ_PROVIDERS__` token, the `__CREDIT_CTA__` copy on the home/omp pages, and `free-credits.gist.md` (regenerated by `npm run build`, written to the repo root, not `dist/`). Card classes live as literals in `credits.mjs` with its own `@source` line in `src/style.css`. `verify.cjs` only checks that each `provider` and its `models` appear on the page and in the gist — not exact copy.
...
- `config.json` models take an optional `note` string; `renderPriceTable` renders it under the model `id`. Currently unused: DeepSeek had planned to route `deepseek-v4-pro` to V4.1 Flash on Sep 14, 2026, but confirmed (email, Sep 2026) it will keep serving V4 Pro past that date at unchanged billing, so the Pro row carries no note and the homepage no longer has a retirement callout.
